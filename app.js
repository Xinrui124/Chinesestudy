 ...
 * 全班同榜：Google Apps Script（已配置 URL）
 *************************************************/

/** ✅ 你的全班同榜（Google Apps Script Web App URL） */
...
const QUESTION_BANK = [
  ...
  { id:"ZC-U1-GS-013", topic:"字词", unit:"第一单元", lesson:"古诗二首", type:"mcq",
    ...
    options:["二月春风似剪刀","三月春风像剪刀","二月春风如小刀","二月春风剪细叶"], answerIndex:0 },
...
const LS = {
  NAME: "ywxzy_name_v4",
  LOCAL_DAILY: "ywxzy_local_daily_scores_v4"
};
...
function setLocalDailyMap(obj) { localStorage.setItem(LS.LOCAL_DAILY, JSON.stringify(obj)); }
...
// ====== 远程榜单（全班同榜）======
function hasRemote() { return REMOTE_LEADERBOARD_URL && REMOTE_LEADERBOARD_URL.trim().length > 0; }
...
function ensureNameOrToast() {
  const nm = getName();
  if (!nm) { alert("请先输入姓名并确认～"); showView("home"); return null; }
  return nm;
}
...
function startLesson(lessonName) {
  ...
  list = QUESTION_BANK.filter(q => q.topic === selectedTopic && q.unit === selectedUnit && q.lesson === selectedLesson);
  ...
  list = shuffle([...list]);
  i = 0; roundScore = 0; unlockedNext = false;
...
async function passQuestion() {
  ...
  if (hasRemote() && (!r || r.ok !== true)) {
    ...
    feedback.textContent = `✅ 答对啦！+1 分（远程榜单提交可能失败，稍后在榜单页点“刷新榜单”）`;
  } else {
    feedback.className = "feedback ok";
    feedback.textContent = `✅ 答对啦！+1 分（已提交到全班同榜）`;
  }
...
function nextQuestion() {
  ...
  if (i < total - 1) { i += 1; renderQuestion(); return; }
...
async function renderBoard() {
  ...
  boardStatus.textContent = "正在加载全班同榜…";
  ...
  boardStatus.textContent = "远程榜单加载失败（请确认 Apps Script 部署权限为“任何人”）";
  // 兜底：本机
  const map = getLocalDailyMap();
  const t = todayKey();
  const today = map[t] || {};
  ...
  champSub.textContent = `今日积分：${topScore} 分（本机兜底）`;
